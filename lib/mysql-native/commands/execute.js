...
function execPrepared(sql, parameters)
{
  var exec = new cmd(
  {
      start: function()
      {
        ...
        if (parameters)
	{
	  parameters = parameters.map(function(p){ return p instanceof Buffer ?
	    p.toString('binary') : p });
	}
        ...
        {
          var nb = calc_null_bitmap(parameters)
	      ...
	      {
		packet.lcstring(parameters[i].toISOString()
		  .replace(/^(.*)T(.*)\..*Z/, '$1 $2'))
	      }
	      else
	      {
      ...
      binrow: function( r )
      {
        ...
        for (var f=0; f < this.ps.field_count; ++f)
        {
            ...
            if (!null_bit_map[f])
            {
                var value = r.unpackBinary(field.type, field.flags & field_flags.UNSIGNED, field.flags & field_flags.BINARY);
