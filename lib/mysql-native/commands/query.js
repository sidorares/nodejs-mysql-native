...
var pack = require('../serializers/pack');
...
module.exports = function(sql, callback)
{
  var c = new cmd(
  {
    ...
    rs_ok: function( r )
    {
      ...
      this.insertId = ok.insert_id;
      this.affectedRows = ok.affected_rows; // protocol decoding side uses names from protocol & mysql naming 
  
      if (ok.field_count == 0)
      {
          if (this.callback)
              this.callback(null, this.resultset);
          return 'done';
      }
    ...
    data: function( r )
    {
        ...
        if (this.callback)
            this.resultset.push(row)
        this.emit('row', row_arr);
    }
  });

  c.resultset = {};
  c.callback = callback;
  c.command_name = 'query';
  return c;
}
